...
{
	public static final IBlockState LOGS = ModBlocks.LOGS.getDefaultState().withProperty(BlockLog1.VARIANT, EnumHandlerPlanks1.EnumTypePlanks1.CHERRY);
	public static final IBlockState LEAF = ModBlocks.LEAF.getDefaultState().withProperty(BlockLeaves1.VARIANT, EnumHandlerPlanks1.EnumTypePlanks1.CHERRY);
		...
		else
		{
			...
			if (isSoil && y < world.getHeight() - height - 1)
			{
				...
				for (int logHeight = 0; logHeight < height; logHeight++)
				{
					...
					{
						this.setBlockAndNotifyAdequately(world, pos.up(logHeight), LOGS);
					}
				}
